...
'''MambaInnerFn 和 MambaInnerFnNoOutProj 两个类都是继承自 torch.autograd.Function 的自定义前向和反向传播函数，
它们的主要区别在于是否包含输出投影（out projection）。'''
...
'''bimamba_inner_fn 函数是一个包装器函数，它调用了 BiMambaInnerFn 类的 apply 方法来实现双向扫描的计算。
该函数接受多个参数，这些参数用于双向扫描过程中所需的各种计算'''
...
'''apply 方法是 torch.autograd.Function 类的一个类方法。它是自定义函数的入口，用于调用自定义的前向和反向传播逻辑
当调用 apply 方法时，PyTorch 会执行以下步骤：
1.调用 forward 方法：
2.调用 backward 方法：

通过调用 apply 方法，PyTorch 自动执行了自定义前向传播和反向传播逻辑。这种机制允许我们定义复杂的计算，并在自动求导系统中使用它们，确保梯度正确传播。
'''
